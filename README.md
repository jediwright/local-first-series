...
A series of working prototypes demonstrating local-first architecture applied to several domains: governance monitoring, commerce, healthcare, and social networking. Plus a new entry — a published architectural specification, not yet built — that extends the same argument into the employment relationship, with deliberate scope toward multi-personal and multi-professional applications.
...
The fifth entry is the first in the series where the specification preceded the reference implementation. The spec is at [employment-seam](https://github.com/jediwright/employment-seam); the reference implementation (Automerge + Keyhive substrate, v0.5 build plan) is built and passing.
...
Vocabulary and schema artifacts the Seam Stack depends on. The base crossing-record vocabulary and employment-seam vocabulary resolve to files served from [`jediwright/seam-stack`](https://github.com/jediwright/seam-stack) via GitHub Pages:

| Namespace | IRI |
| --------- | --- |
| Crossing record (base shape) | `https://jediwright.github.io/seam-stack/vocab/crossing-record/0.1/` |
| Employment seam | `https://jediwright.github.io/seam-stack/vocab/employment-seam/0.5/` |
| Canonical assurance scale | `https://jediwright.github.io/seam-stack/vocab/assurance/` |

The SHACL shapes, JSON-LD context, and JSON Schema fallback validator for the employment seam bundle live alongside the vocabulary. These artifacts are referenced by the employment seam spec and are intended to be reusable beyond it.
...
**0. The governed crossing.** The abstract pattern that all prior entries have been instantiating. A governed crossing is the boundary event at which a party with contextual knowledge crosses into or out of a structured relationship under a capability grant. The architectural argument: every such crossing must be explicit, gated, and recorded before it fires, with the platform facilitating the event and exiting rather than accumulating the relationship. Four constitutive properties — declared scope, grant, gate, record — are invariant across all domain instantiations. Everything else (participant model, failure taxonomy, legal substrate, bundle schema, identity ceremony) varies by domain and is specified in the domain entry. Spec at [`pattern-commons-00-the-governed-crossing`](https://github.com/jediwright/local-first-series/blob/main/pattern-commons-00-the-governed-crossing-v0-1.md).
...
The Seam Stack as an architectural composition is documented at [seamstack.org](https://seamstack.org) and in the [`seam-stack`](https://github.com/jediwright/seam-stack) repository. The seventh Pattern Commons entry — the employment seam — is the spec where the four-layer synthesis is demonstrated end-to-end. Pattern Commons #0 names the abstract pattern that the series has been building toward. The companion entries (#1–#6) demonstrate components of the synthesis across the prior built prototypes.
