...
**#00 — The governed crossing.** The abstract pattern that all prior entries have been instantiating. A governed crossing is the boundary event at which a party with contextual knowledge crosses into or out of a structured relationship under a capability grant. Four constitutive properties — declared scope, grant, gate, record — are invariant across all domain instantiations. The architectural argument: every such crossing must be explicit, gated, and recorded before it fires, with the platform facilitating the event and exiting rather than accumulating the relationship. Spec at [pattern-commons-00](./pattern-commons/pattern-commons-00-the-governed-crossing.md).
...
**#7 — The employment seam.** The boundary event when a person enters or exits an employer–worker relationship. Worker owns the knowledge graph; the platform facilitates the handoff and exits. Bundle schema (SHACL over RDF, JSON-LD canonical), legal record format (W3C Verifiable Credentials 2.0 + RFC 3161 + OpenTimestamps), nine-state failure taxonomy, multi-perspective record in contested cases, seven-class participant model including AI agents (Class G). Generalizes WARN Act, EU European Works Council Directive, EU Collective Redundancies Directive, and contractor classification regimes. The first Pattern Commons entry where all four Seam Stack layers become necessary at once. Spec at [pattern-commons-07](./pattern-commons/pattern-commons-07-employment-seam.md). Reference implementation at [employment-seam](https://github.com/jediwright/employment-seam).

**#8 — The substrate-crossing seam.** The boundary event when a local-first system crosses into a substrate with different properties — different revocation semantics, different exposure guarantees, different epistemic regime. Where the employment seam crosses within a single substrate, the substrate-crossing seam crosses *between* substrates: the seam-side architecture enforces up to the boundary, then terminates by declaration. The pattern introduces a two-record intent/completion discipline (the crossing is declared before it fires, recorded after it lands), a new `exposure-unbounded` bound type (the architecture cannot assert an upper bound on post-crossing exposure), and a six-state failure taxonomy for crossing attempts. First worked domain: AT Protocol (Bluesky) as target substrate. Prototype-verified on that substrate (Phases 0–3, eight governed runs against a live PDS; reference implementation at [employment-seam](https://github.com/jediwright/employment-seam)). Spec at [pattern-commons-08](./pattern-commons/pattern-commons-08-substrate-crossing-seam.md).

**#9 — The governed content production crossing.** The boundary event when a content object crosses from an author-controlled local-first substrate to an externally legible surface — a CMS, a public AT Protocol record, a publication endpoint — at a declared TCF tier-compliance threshold. Where prior seams govern *who* crosses and *what* transfers, this seam governs the *governed state of the content object itself* at crossing time. The gate checks TCF Quark-level constraint satisfaction and `tcf:register` before the publish call fires; the crossing record is the schema-checkable artifact that makes TCF tier boundaries machine-verifiable rather than editorial. Design intent, speculative. Spec at [pattern-commons-09](./pattern-commons/pattern-commons-09-governed-content-production-crossing.md).
