...
A series of working prototypes demonstrating local-first architecture applied across five domains: governance monitoring, commerce, healthcare, social networking, and employment. Each entry introduces a harder version of the same problem: something irreducibly server-dependent has to happen. The question is not whether to touch a server — sometimes you must. The question is how to design that boundary deliberately, minimize it, and ensure the client loses nothing when it fails.
...
The series demonstrates a four-layer architecture for local-first systems where boundary events carry weight. I call it the **Seam Stack**.

| Layer | Question it answers | Technology in this series |
|---|---|---|
| **Substrate** | Where does the data live, who owns it, who can access it? | Y.js + IndexedDB (prototypes 1–4); Automerge + Keyhive (employment seam — authorization-backed; capability grant and revocation are first-class primitives) |
...
| **Evidence** | What makes the record contemporaneous, tamper-evident, and legible to deferred parties without platform mediation? | W3C Verifiable Credentials Data Model 2.0 + RFC 3161 trusted timestamping + OpenTimestamps + bilateral cryptographic signatures (employment seam); `seam:CrossingRecord` base shape + governed instance extensions |

The synthesis claim is that all four layers are required when boundary events carry legal and evidentiary weight, and that they compose into a coherent architecture for local-first systems in those domains. None of the four layers is novel in isolation; the synthesis is the architectural composition.
...
| 5 | employment-seam | One seam per transition (entry, exit, stage change, re-engagement) | Built — v0.5 spec + reference prototype | [employment-seam](https://github.com/jediwright/employment-seam) |

Each prototype introduces a harder version of the seam problem. The Tracker has no seam at all. checkout-seam has one seam per transaction. fhir-seam introduces a richer failure taxonomy and higher stakes. Local-First Social has a seam that fires on every new connection — the social graph itself is a distributed seam. The employment seam fires on every transition in the employer–worker relationship, with nine failure states, multi-perspective preservation in contested cases, a legal record format designed for evidentiary use across jurisdictions, and capability-backed authorization where revocation is a first-class architectural event.

The fifth entry is the first in the series built on an authorization-backed substrate (Automerge + Keyhive) and the first to include a governed crossing-record architecture: a `seam:CrossingRecord` base shape with four instance extensions — gate-check, AI provenance, GSEF lineage, and GPRF verification — plus a P13 evidence plane for multi-party governance (amendment proposals, objections, consent, and resolutions as governed record types). 131 tests passing.
...
**7. The employment seam.** *Pattern Commons #7, v0.5 — spec and reference prototype.* The boundary event when a person enters or exits an employer–worker relationship. Worker owns the knowledge graph; the platform facilitates the handoff and exits. Authorization-backed substrate (Automerge + Keyhive): capabilities are granted at entry and revoked at exit as a first-class seam event. `seam:CrossingRecord` base shape with four governed instance extensions and P13 multi-party governance machinery (amendment, objection, consent, resolution). Nine-state failure taxonomy, multi-perspective record in contested cases, seven-class participant model. Generalizes WARN Act, EU European Works Council Directive, EU Collective Redundancies Directive, and contractor classification regimes. The first Pattern Commons entry where all four Seam Stack layers become necessary at once. Spec and prototype at [employment-seam](https://github.com/jediwright/employment-seam).

---

## Shared Specifications

These files govern the prototypes. They are series artifacts, not prototype-specific. Every build session reads them before writing code.

### `DESIGN.md`

Design tokens and rationale for the series. YAML front matter contains normative values for colors, typography, spacing, and component tokens. The seam state palette (`seam-none`, `seam-connecting`, `seam-established`, `seam-error`) is the load-bearing design decision — it makes the architectural argument visible in the UI.
...
Vocabulary and schema artifacts the Seam Stack depends on. The vocabulary at `seamstack.org/vocab/crossing-record/0.1#` resolves to files served from this directory via GitHub Pages. The SHACL shapes, JSON-LD context, and JSON Schema fallback validator for the employment seam bundle live alongside the vocabulary.
...
The full account of what was built, what broke, what the AI wrote that shouldn't have shipped, and what the governance framework is: [Systems of Thought](https://www.systemsofthought.com/).

The Seam Stack as an architectural composition: [seamstack.org](https://seamstack.org). The seventh Pattern Commons entry — the employment seam — is the spec where the four-layer synthesis is demonstrated end-to-end. The companion entries (#1–#6) demonstrate components of the synthesis across the prior built prototypes.

The governance argument the architecture serves: [Full Personhood: The Governance Model AI Requires and Capitalism Never Built](https://www.systemsofthought.com/full-personhood/).
